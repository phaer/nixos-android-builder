...
Environment variables:
    KEYLIME_ENROLL_URL      Base URL of the enrollment server
                            (default: from /boot/attestation-server.json)
...
def get_enroll_url() -> str:
    """Determine the enrollment server URL."""
    env_url = os.environ.get("KEYLIME_ENROLL_URL")
    if env_url:
    ...
    if not ATTESTATION_SERVER.exists():
        print(
            f"Error: {ATTESTATION_SERVER} not found and"
            " KEYLIME_ENROLL_URL not set.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    ...
    # Skip TLS verification — the enrollment server doesn't
    # require mTLS and may use a self-signed cert.
